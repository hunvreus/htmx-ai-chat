...
app = FastAPI(title="Step 3 - SQLite Persistence")
...
async def create_message(request: Request, message: str = Form(...)):
    ...
    history = [{"role": row["role"], "content": row["content"]} for row in get_messages()]
    answer = ask_llm(history)

    with db() as conn:
        conn.execute("INSERT INTO messages(role, content) VALUES('assistant', ?)", (answer,))
